...
class ContentExtractor(object):
    ...
    def prettify(self, html):
        ...
        for elem in soup.findAll('embed'): #[@type="application/x-shockwave-flash"]'):
            if elem.get('type') != 'application/x-shockwave-flash':
                continue
            idx = len(self.flashes)
            src = elem.get('src', '')
            if src:
                self.flashes.append(src)
                try:
                    width = int(elem.get('width', 1))
                    height = int(elem.get('height', 1))
                except ValueError:
    ...
    def get_tag_ratio_list(self, html):
        ...
        html = strip_tags(html, tag='code')  # we don't need code
        html = strip_comments(html)
        
        title, html= self.prettify(html)
        lines = html.split('\n')
    ...
    def get_content(self, title, content_lines):
        dp = ContentParser()
    ...
    def kmean_cluster(self, html, k=5):
        assert k >= 2
        title, tag_ratio_list = self.get_tag_ratio_list(html)
        ...
        return self.get_content(title, content_lines)
...
def test():
    ...
    title, contents = extractor.extract_content(data)
    print 'title:', title
